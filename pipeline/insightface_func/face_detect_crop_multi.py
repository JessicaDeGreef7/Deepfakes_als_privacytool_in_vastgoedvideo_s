...
class Face_detect_crop:
    ...
    def get(self, img, crop_size, detecteerde_gezichten_frame = [], max_num=0):
        #print("hey")
        align_img_list = []
        M_list = []
        if detecteerde_gezichten_frame == []:
            ...
            #if len(detecteerde_gezichten_frame) == 1:
            bboxes = detecteerde_gezichten_frame[1]
            kps = np.array(detecteerde_gezichten_frame[2])
            M, _ = face_align.estimate_norm(kps, crop_size, mode = 'None') 
            align_img = cv2.warpAffine(img, M, (crop_size, crop_size), borderValue=0.0)
            align_img_list.append(align_img)
            M_list.append(M)
            #else:
            #    aantal_gezichten = len(detecteerde_gezichten_frame)
            #    print(detecteerde_gezichten_frame)
            #    for i in range(aantal_gezichten):
            #        print("-detcet-")
            #        print(detecteerde_gezichten_frame)
            #        kpss = np.array(detecteerde_gezichten_frame[2])
            #        kps = None
            #        if kpss is not None:
            #            kps = kpss[i]
            #        M, _ = face_align.estimate_norm(kpss, crop_size, mode = self.mode) 
            #        align_img = cv2.warpAffine(img, M, (crop_size, crop_size), borderValue=0.0)
            #        align_img_list.append(align_img)
            #        M_list.append(M)
